...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _buildCategories(ItemsViewModel vm, String locale) {
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: const [
                            Icon(Icons.more_horiz, color: Colors.orange, size: 32),
                            SizedBox(height: 12),
                            Text(
                              'See More',
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              gradient: LinearGradient(
                                colors: [HomeScreen._primaryColor, HomeScreen._primaryColor.withOpacity(0.8)],
                              ),
                              borderRadius: BorderRadius.circular(16),
                            ),
                            child: const Icon(
  ...
  Widget _buildItemsHorizontal(List items, BuildContext context, String locale) {
    if (items.isEmpty) {
      return Container(
        height: 280,
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            const Icon(Icons.more_horiz, color: Colors.orange, size: 40),
                            const SizedBox(height: 12),
                            Text(
                              'see_more'.tr(),
                              textAlign: TextAlign.center,
              ...
              return Transform.translate(
                ...
                child: Opacity(
                  ...
                  child: Container(
                    width: 180,
                    margin: const EdgeInsets.only(right: 16),
                    child: _buildModernProductCard(context, item, locale),
                  ),
                ),
              );
            },
  ...
  Widget _buildModernProductCard(BuildContext context, item, String locale) {
            ...
            builder: (context, vm, child) {
              ...
              return ListView(
                children: [
                  ...
                  _buildCarousel(),
                  _buildCategories(vm, locale),
                  const SizedBox(height: 32),
                  _buildSectionTitle('latest_items'.tr()),
                  _buildItemsHorizontal(vm.items, context, locale),
                  const SizedBox(height: 24),
                  _buildSectionTitle('best_sellers'.tr()),
                  _buildItemsHorizontal(vm.bestSellers, context, locale),
                  const SizedBox(height: 24),
                ...
                Flexible(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'welcome_back'.tr(),
                        style: TextStyle(color: Colors.white70, fontSize: MediaQuery.of(context).size.width * 0.04),
                        overflow: TextOverflow.ellipsis,
                      ),
                      Text(
                        'lets_go_shopping'.tr(),
          ...
          RawAutocomplete<String>(
            optionsBuilder: (TextEditingValue textEditingValue) {
              ...
              final categoryMatches = vm.categories
                  .where((cat) => (locale == 'ar' ? cat.nameAr : cat.nameEn).toLowerCase().contains(query))
                  .map((cat) => locale == 'ar' ? cat.nameAr : cat.nameEn);
        final productMatches = vm.items
          .where((item) => (locale == 'ar' ? item.nameAr : item.nameEn).toLowerCase().contains(query))
          .map((item) => locale == 'ar' ? item.nameAr : item.nameEn);
            ...
            onSelected: (String selection) async {
              final categoryMatch = vm.categories.firstWhere(
                (cat) => (locale == 'ar' ? cat.nameAr : cat.nameEn).toLowerCase() == selection.toLowerCase(),
                orElse: () => Category(id: -1, nameEn: '', nameAr: ''),
              );
              if (categoryMatch.id != -1) {
                await vm.filterByCategory(categoryMatch.id.toString());
                if (vm.items.isEmpty && !vm.loading && context.mounted) {
                  _showNoResultsDialog(context, "${locale == 'ar' ? 'الفئة' : 'Category'}: ${locale == 'ar' ? categoryMatch.nameAr : categoryMatch.nameEn}");
                  await vm.loadItems();
                }
              } else {
                vm.searchItems(selection);
                if (vm.items.isEmpty && !vm.loading && context.mounted) {
                  _showNoResultsDialog(context, "${locale == 'ar' ? 'المنتج' : 'Product'}: $selection");
                  await vm.loadItems();
                }
              }
            },
            fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
              return TextField(
                ...
                decoration: InputDecoration(
                  hintText: locale == 'ar' ? "ابحث عن منتج أو فئة..." : "Search for a product or category...",
                  prefixIcon: Icon(Icons.search, color: HomeScreen._primaryColor),
                  border: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(16),
                    borderSide: BorderSide.none,
                  ),
                ...
                child: Material(
                  ...
                  child: ListView(
                    ...
                    children: options.map((option) {
                      return ListTile(
                        title: Text(option, textDirection: locale == 'ar' ? ui.TextDirection.rtl : ui.TextDirection.ltr),
